...
src/
    ...
    Makefile, Makefile.Android   # build scripts
```
...
You can use this templates in a few ways: using Visual Studio, using the raw Makefile, or make your own build setup. This repository comes with Visual Studio already set up.

Chose one of the follow setup options that fit in you development environment.

### CLI: Helper scripts (recommended on Linux)
...
`test.sh` runs the unit test suite (see [ADR-0006](docs/adr/0006-doctest-for-unit-tests.md)) the
...
`build.sh`, it doesn't need raylib or EnTT vendored first — the systems under test today
...
Build raylib itself for the web target, then the game, both via the same Makefile used for desktop
(there's no separate Web build system in this repo):
