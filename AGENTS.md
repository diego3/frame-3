...
Most existing files in `src/game/` came from the raylib game template and are currently C (`.c`) files. Treat them as transitional template code: refactor and replace them progressively with C++ that follows the engine's architecture and conventions, rather than extending the template's ad-hoc screen logic indefinitely. `src/game/` is the extension layer for a concrete game; each game belongs there, while `src/app/` is the reusable game engine and must not depend on, include, or otherwise know game-specific types, assets, rules, screens, or behavior.
