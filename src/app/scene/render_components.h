...
//
// NOTE: coexists with Renderable (renderable.h, game/flare_reactor's shape/size/color/wireframe
// component, RFC-0001 Phase 1) rather than converging into one type -- both landed independently
// on the same underlying concern ("what does this entity look like"), renderable.h's own comment
// flagged the option to converge once this branch merged, but doing so is real new design
// (indexing/discovery across render-component types, not just a rename) and belongs to a
// dedicated ADR, not a side effect of reconciling two branches' file layouts.
